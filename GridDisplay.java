...
public class GridDisplay extends JComponent implements KeyListener, MouseListener
{
    ...
    private void init(int numRows, int numCols)
    {
        ...
        cellSize = Math.max(1, Math.min(MAX_SIZE / getNumRows(), MAX_SIZE / getNumCols()));
    ...
    {
        setColorWithoutRepaint(loc, color);
        repaint();
    }

    public void setColorWithoutRepaint(Location loc, Color color)
    {
        if (!isValid(loc))
